...
/*  Created On : 07-sept.-2019 9:01:31 a. m. 				*/
...
IF EXISTS (SELECT 1 FROM dbo.sysobjects WHERE id = object_id(N'[FK_Usuario_TipoUsuario]') AND OBJECTPROPERTY(id, N'IsForeignKey') = 1) 
...
CREATE NONCLUSTERED INDEX [IXFK_Usuario_TipoUsuario] 
 ON [Usuario] ([tipoUsuarioId] ASC)
GO
...
ALTER TABLE [Usuario] ADD CONSTRAINT [FK_Usuario_TipoUsuario]
	FOREIGN KEY ([tipoUsuarioId]) REFERENCES [TipoUsuario] ([id]) ON DELETE No Action ON UPDATE No Action
GO
